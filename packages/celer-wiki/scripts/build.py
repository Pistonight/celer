...
def process_page_text(text, dir_path, is_release, abs_path_map):
    """Process text, replace image and clickable links"""
    out_parts = []
    state = 0
    # state machine
    # states:
...
def get_replacement(dir_path, is_link, name, link, is_release, abs_path_map):
    """Get replacement string for link or image"""
...
def build_for_file(dir_path, dir_stack, file_name, abs_path_map, is_release, order_maps):
    ...
    with open(os.path.join(dir_path, file_name), "r", encoding="utf-8") as input_file:
        ...
        if key in order_maps["prev"]:
            prev_file = order_maps["prev"][key]
        next_file = None
        if key in order_maps["next"]:
            next_file = order_maps["next"][key]

        output_lines = build_file(
            input_file,
            dir_path,
            dir_stack,
            file_name,
...
def build_file(
    ...
    prev_file,
    next_file):
    """Build the content of non-index page"""
    output = []
    if dir_stack:
    ...
    if prev_file is not None or next_file is not None:
        ...
        if prev_file is not None:
            prev_link = urllib.parse.quote(prev_file)
            prev_name = file_path_to_title(prev_file)
            output.append(
                process_page_text(
                    f"Previous: [{prev_name}](./{prev_link})",
                    dir_path,
                    is_release,
                    abs_path_map))

        if prev_file is not None and next_file is not None:
            output.append(" | ")

        if next_file is not None:
            next_link = urllib.parse.quote(next_file)
            next_name = file_path_to_title(next_file)
            output.append(
                process_page_text(
                    f"Next: [{next_name}](./{next_link})",
                    dir_path,
                    is_release,
                    abs_path_map))
...
def convert_file_name(dir_stack, file_name):
    """Convert file name in directory structure to a flattened, bracket notation file name"""
...
def create_top_nav(dir_stack, title, dir_path, is_release, abs_path_map):
    """Create markdown for top navigation bar"""
